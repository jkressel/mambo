...
#define TRACE_EXIT_BLOCK_SIZE 1000
...
size_t scan_trace(dbm_thread *thread_data, uint16_t *address, cc_type type, int *set_trace_id) {
    ...
    if ((write_p - (uint8_t *)thread_data->code_cache_meta[trace_id].tpc >= WHOLE_TRACE - TRACE_EXIT_BLOCK_SIZE) ||
        ((thread_data->trace_fragment_count+1) * 20) >= TRACE_EXIT_BLOCK_SIZE) {
      return 0;
    }
